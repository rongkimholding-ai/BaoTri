<div class="card-body p-0">
    ...
    <div class="d-none d-md-block">
        <div class="card">
            <div class="table-scroll-top-system"><div></div></div>
                    ...
                    <tbody>
                        @forelse($items as $item)
                            @php
                                ...
                                // Safety for workType array key
                                $workTypeDisplay = isset($workType[$item->work_type]) ? $workType[$item->work_type] : $item->work_type;

                                // Optimize acceptanceList access
                                $acceptanceResultKey = $item->acceptance_result;
                                $acceptanceResultValue = (!empty($acceptanceResultKey) && isset($acceptanceList[$acceptanceResultKey])) 
                                    ? $acceptanceList[$acceptanceResultKey] 
                                    : (empty($acceptanceResultKey) ? '' : $acceptanceResultKey);
                            @endphp
                            <tr data-id="{{ $item->id }}" class="tr-row-link" data-detail-url="{{ $detailRoute }}">
                                ...
                                <td style="min-width: 100px;max-width: 150px;">
                                    @php
                                        ...
                                        if (!empty($item->standard_completion_time)) {
                                            ...
                                            if ($configItem && isset($configItem['name'])) {
                                ...
                                <td class="text-center">{{ $acceptanceResultValue }}</td>
                                                    ...
                                                    @foreach ($nextStatuses as $nextStatus)
                                                        ...
                                                        <li>
                                                            <a href="javascript:void(0)"
                                                                ...
                                                                class="dropdown-item">
                                                                {{ isset($statusNamesHt[$nextStatus]) && !empty($nextStatus) ? $statusNamesHt[$nextStatus] : str_replace('_', ' ', $nextStatus) }}
                                                            </a>
                                                        </li>
                                                        @endif
                                                    @endforeach
        ...
        <div class="row g-3">
            @forelse($items as $item)
                @php
                    ...
                    // Safe for workType, acceptanceList
                    $workType = config('work_type.name');
                    $workTypeDisplay = isset($workType[$item->work_type]) ? $workType[$item->work_type] : $item->work_type;
                    $acceptanceResultKey = $item->acceptance_result;
                    $acceptanceResultValue = (!empty($acceptanceResultKey) && isset($acceptanceList[$acceptanceResultKey])) 
